...
| Disappearing messages: on by default, max 1 week | ⬜ design | Benji's ask: always-on disappearing messages, duration selectable up to 7 days max (no off switch). Protocol-level feature — needs design first: TTL enforcement on both ends, existing-conversation migration, offline-delivery interaction. Not started. |
