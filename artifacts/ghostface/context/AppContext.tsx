...
async function registerWithServer(
  ...
  try {
    ...
    // Adopt the JWT pair (task #198). The legacy opaque token is kept as a
    // compatibility credential only.
    await setAuthTokens({
      accessToken: data.accessToken ?? null,
      refreshToken: data.refreshToken ?? null,
      legacyDeviceToken: data.token ?? null,
    });
    return {
      token:        data.token,
...
async function rekeyWithServer(
  ...
  try {
    ...
    // authFetch prefers the JWT access token and auto-refreshes on 401; the
    // passed legacy token is a fallback for pre-JWT devices.
    const res = await authFetch(`${apiBase}/prekeys/${encodeURIComponent(userId)}/rekey`, {
      method: "PUT",
...
async function generateAndUploadOPKs(userId: string, deviceToken: string): Promise<void> {
  ...
  try {
    ...
    const res = await authFetch(`${apiBase}/prekeys/${encodeURIComponent(userId)}`, {
      method: "POST",
      headers: {
...
async function replenishOPKsIfNeeded(userId: string, deviceToken: string): Promise<void> {
  ...
  if (!apiBase) return;
  try {
    const res = await authFetch(`${apiBase}/prekeys/${encodeURIComponent(userId)}/count`, {
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // JWT auth lifecycle (task #198): keep app state in sync with silent
  // token rotations, and route back to onboarding if the refresh token is
  // ever rejected (expired or revoked server-side).
  useEffect(() => {
    setOnTokensRotated((access) => {
      setState((prev) => ({ ...prev, deviceToken: access }));
    });
    setOnAuthFailure(() => {
      AsyncStorage.removeItem("isOnboarded").catch(() => {});
      setState((prev) => ({ ...prev, deviceToken: null, isOnboarded: false }));
    });
    return () => {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Hydrate the JWT auth module (task #198). `deviceToken` in app
        // state carries the current Bearer credential — the JWT access
        // token when present, the legacy opaque token otherwise. authFetch
        // keeps it fresh via /auth/refresh on 401.
        const authTokens = await loadAuthTokens();
        const bearerToken = authTokens.accessToken ?? storedToken ?? null;
            ...
            try {
              ...
              if (!token) {
                ...
                if (reg) {
                  ...
                  setState((prev) => ({ ...prev, deviceToken: reg.accessToken ?? reg.token }));
  ...
  const panicWipe = useCallback(async () => {
    ...
    // Server-side session teardown (task #198): best-effort revoke of the
    // refresh token, then drop the stored JWT pair. Never blocks the wipe.
