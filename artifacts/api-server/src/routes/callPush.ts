...
router.post("/calls/register-voip-token", async (req: Request, res: Response) => {
  try {
    ...
    // ── Bearer auth (unified JWT + legacy, same scheme as prekeys + WS) ──
    const bearer = bearerToken(req);
    if (!bearer) {
      return res.status(401).json({ error: "Authorization: Bearer <token> header required" });
    }
    const authed = await resolveBearerUser(bearer, userId);
    if (!authed) {
      return res.status(403).json({ error: "Invalid, expired, or mismatched token for alias" });
    }
